...
public class ChangePassword extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final String forgot_password_prof_id = sharedPreferences.getString("forgot_password_prof_id", "DNE");

        if (forgot_password_prof_id.equals("DNE")) {
            finish();
        }
            ...
            public void onClick(View view)
            {
                ...
                if(new_pass.length() != 0 && confirm_pass.length() != 0 && new_pass.equals(confirm_pass))
                {
                            ...
                            connectivityManager.getNetworkInfo(ConnectivityManager.TYPE_WIFI).getState() == NetworkInfo.State.CONNECTED)
                    {
                        ...
                        try
                        {
                            type = "change_prof_password";

                            String change_prof_passwordResult = new DatabaseActions().execute(type, forgot_password_prof_id, new_pass).get();

                            if(change_prof_passwordResult.equals("Something went wrong"))
                            {
                                change_pass_feed.setText(change_prof_passwordResult);
                            }
                            else if(change_prof_passwordResult.equals("-1"))
                            {
                                change_pass_feed.setText("Database issue found");
                            }
                            else if(change_prof_passwordResult.equals("0"))
                            {
