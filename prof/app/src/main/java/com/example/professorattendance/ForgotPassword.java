    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        else // Permission is already granted
        {
            ...
            done_btn.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view)
                {
                    String username = username_input.getText().toString().toUpperCase();
                    String serial_no = serial_input.getText().toString();

                    if(username.length() != 0 && serial_no.length() != 0)
                    {
                        if(serial_no.equals(uniqueID)) //serial number of that phone is same as that of entered serial number
                        {
                                    ...
                                    connectivityManager.getNetworkInfo(ConnectivityManager.TYPE_WIFI).getState() == NetworkInfo.State.CONNECTED)
                            {
                                try
                                {
                                    type = "check_prof_username_and_serial_exist_in_db";
                                    String check_prof_username_and_serial_exist_in_dbResult = new DatabaseActions().execute(type, username, serial_no).get();

                                    if(check_prof_username_and_serial_exist_in_dbResult.equals("-1"))
                                    {
                                        forgot_pass_feed.setText("Database issue found");
                                    }
                                    else if(check_prof_username_and_serial_exist_in_dbResult.equals("Something went wrong"))
                                    {
                                        forgot_pass_feed.setText(check_prof_username_and_serial_exist_in_dbResult);
                                    }
                                    else if(Integer.parseInt(check_prof_username_and_serial_exist_in_dbResult)> 0)
                                    {
                                        //creating cookie of the forgot_password_stud_id
                                        SharedPreferences.Editor editor = sharedPreferences.edit();
                                        editor.putString("forgot_password_prof_id", check_prof_username_and_serial_exist_in_dbResult);
                                        editor.apply();
